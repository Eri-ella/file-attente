...
<body class="h-full flex flex-col">
    ...
    {{-- Ticket appelé (compte à rebours 5 min) --}}
    @if($ticketAppele)
        <div class="text-center bg-yellow-600 text-white py-6 px-4 font-medium">
            ...
            <div class="mt-4 space-x-2">
                <form action="{{ route('manager.ticket.demarrer', $ticketAppele) }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="bg-white text-yellow-700 px-6 py-2 rounded font-bold">Démarrer le traitement</button>
                </form>
                <form action="{{ route('manager.ticket.noShow', $ticketAppele) }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded font-bold">Absent</button>
                </form>
            </div>
        </div>
    @endif
        ...
        <div class="bg-[#F9F8F5]">
            ...
            <div class="text-center pb-6">
                <form action="{{ route('manager.ticket.terminer', $ticketEnCours) }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-8 py-3 rounded font-bold text-lg">Terminer le ticket</button>
                </form>
            </div>
        </div>
    @else
                ...
                <table class="w-full text-left text-sm">
                    <thead>
                        <tr class="text-gray-500 border-b border-[#222D52]/20">
                            ...
                            <th class="pb-2 font-normal">Retards</th>
                    ...
                    <tbody>
                        @forelse ($file as $t)
                            <tr class="border-b border-[#222D52]/10">
                                ...
                                <td class="py-3">
                                    ...
                                    @elseif($t->statut === 'retard_decale')
                                        <span class="bg-orange-100 text-orange-700 px-2 py-1 text-xs">DÉCALÉ</span>
                                    @else
                                        <span class="text-gray-600 text-xs">EN FILE</span>
                                    @endif
                                </td>
                                <td class="py-3 text-center">{{ $t->nombre_retards }}/3</td>
                                <td class="py-3">
                                    @if($t->statut === 'en_file')
                                    ...
                                    @elseif($t->statut === 'en_cours')
                                        <form action="{{ route('manager.ticket.terminer', $t) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" class="bg-green-600 text-white px-3 py-1.5 text-sm rounded">Terminer</button>
                                        </form>
                                    @elseif($t->statut === 'appele')
                                        <form action="{{ route('manager.ticket.demarrer', $t) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" class="border border-[#222D52]/50 px-3 py-1.5 text-sm hover:bg-gray-50">Démarrer</button>
                                        </form>
                        ...
                        @empty
                            <tr><td colspan="7" class="py-6 text-center text-gray-500">File vide</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    <script>
        (function() {
            function initCompteur(id) {
                ...
                if(s>0){c.textContent=fmt(s);setInterval(tick,1000);}else{c.textContent='00:00:00';}
            }
            initCompteur('compteur-manager');
            initCompteur('compteur-appel');
