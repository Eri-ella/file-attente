...
<body>
    ...
    <main class="flex w-full">
        ...
        <div class="flex flex-col items-start justify-center w-1/2 min-h-screen gap-2 text-(--primary-color) px-35">
            ...
            <form action="{{ route('manager.profil.update') }}" method="POST" class="w-full">
                @csrf
                @method('PUT')
                ...
                <div class="flex flex-col gap-3 w-full">
                    ...
                    <input class="border-1 border-(--primary-color) p-2 rounded-lg bg-(--white-color) w-full" type="text" name="nom" value="{{ old('nom', $manager->nom) }}" required>

                    <label class="text-[#222D52] font-medium">Prénom</label>
                    <input class="border-1 border-(--primary-color) p-2 rounded-lg bg-(--white-color) w-full" type="text" name="prenom" value="{{ old('prenom', $manager->prenom) }}" required>

                    <label class="text-[#222D52] font-medium">Email</label>
                    <input class="border-1 border-(--primary-color) p-2 rounded-lg bg-(--white-color) w-full" type="email" name="email" value="{{ old('email', $manager->email) }}" required>

                    <label class="text-[#222D52] font-medium">Téléphone</label>
                    <input class="border-1 border-(--primary-color) p-2 rounded-lg bg-(--white-color) w-full" type="text" name="numero" value="{{ old('numero', $manager->numero) }}" required>
            ...
            <form action="{{ route('manager.profil.delete') }}" method="POST" onsubmit="return confirm('Supprimer votre compte ? Cette action est définitive.');" class="mt-10">
                @csrf
                @method('DELETE')
