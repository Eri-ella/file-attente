...
<body >
    ...
    <main class="flex w-full">
        ...
        <form class="flex flex-col items-start justify-center w-1/2 min-h-screen gap-2 text-(--primary-color) px-35" action="{{ route('manager.motdepasse.send') }}" method="POST">
            @csrf
            <p class="text-xs tracking-widest text-(--highlight-color) mb-5">GESTION</p>
            ...
            <div class="flex flex-col w-full gap-5 mt-5 items-center justify-center">
                ...
                <a href="{{ route('pageManager') }}">← Retour à la <b>connexion</b></a>
            </div>
        </form>
    </main>
    @if (session('status'))
