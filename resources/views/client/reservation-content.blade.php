<section class="text-(--primary-color) p-5">
    <div>
        <button class="btn-secondary mb-10" onclick="window.smartQueueGoBack()">Retour</button>
    </div>

    @if (session('succes') || ($succes ?? false))
        <div class="bg-green-100 text-green-800 text-sm px-4 py-3 rounded mb-6">
            {{ session('succes') ?? $succes }}
        </div>
    @endif

    <form method="POST" action="{{ route('reservation.store', $service->id) }}" class="js-reservation-form">
        @csrf
...
<script>
    function mettreAJourRecapitulatif() {
        const champDate = document.getElementById('date').value;
        ...
        document.getElementById('recap-tickets').textContent = document.getElementById('nombre_tickets').value || '1';
    }
</script>